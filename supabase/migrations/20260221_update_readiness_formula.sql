-- Update readiness formula: coverage × quality
-- New: readiness% = round((rated_skills / total_skills) × (avg_score / 5) × 100)
-- Ready: readiness% >= 80 AND coverage >= 90% AND no skill < 3 AND cat4 avg >= 4
...
BEGIN
  ...
  -- Average of rated skills (0-5 scale)
  SELECT AVG(current_score::NUMERIC) INTO avg_score
  FROM student_skills
  WHERE student_id = NEW.student_id AND current_score > 0;

  -- coverage × quality → percentage
