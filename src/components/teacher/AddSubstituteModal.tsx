...
export function AddSubstituteModal({ open, onOpenChange }: AddSubstituteModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md" dir="rtl">
        <DialogHeader>
          <DialogTitle>הוספת מורה מחליף</DialogTitle>
          <DialogDescription>צור חשבון כניסה למחליף</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          <div className="space-y-2">
            <Label>שם מלא *</Label>
            <Input value={name} onChange={e => setName(e.target.value)} placeholder="שם המחליף" required />
          </div>
          <div className="space-y-2">
            <Label>אימייל *</Label>
