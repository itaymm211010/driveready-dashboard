  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-right">פרטי שיעור</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 text-right">
          ...
          <div className="glass rounded-xl p-3 space-y-1">
            ...
            {lesson.student.phone && (
              <p className="text-sm text-muted-foreground">{lesson.student.phone}</p>
            )}
          ...
          <div className="glass rounded-xl p-3 space-y-1 text-sm">
            ...
            <p><span className="text-muted-foreground">שעה:</span> {lesson.time_start} - {lesson.time_end}</p>
          ...
          {!isCancelled && (
            <div className="flex gap-2 pt-2 border-t border-border/50">
              <Button variant="destructive" size="sm" className="flex-1" onClick={() => { onOpenChange(false); onCancel(lesson); }}>
                <XCircle className="h-4 w-4 ml-1" /> בטל שיעור
              </Button>
